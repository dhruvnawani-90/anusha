...
export default (async (req, res) => {
    ...
    let magicCode = encryptor.encrypt(payload.magicCode);

    await magicCodeColl.doc('magic_code').update({magicCode: magicCode}, {merge: true});

    return res.status(200).json({ success: true, message: "Magic code updated" });
});